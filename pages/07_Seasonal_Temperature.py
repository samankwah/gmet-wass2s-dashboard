...
st.set_page_config(page_title="Seasonal Temperature", page_icon=":thermometer:", layout="wide")

APP_DIR = Path(__file__).parent.parent
DATA_DIR = APP_DIR / "data" / "seasonal_temp"

st.title("Seasonal Temperature Forecast")
st.markdown("Seasonal temperature outlook for Ghana — Below Normal, Near Normal, or Above Normal.")
...
if forecast_type == "Deterministic":
    if nc_det.exists():
        try:
            ...
            fig = go.Figure(data=go.Heatmap(
                ...
                colorscale="RdYlBu_r", colorbar=dict(title="Temperature (C)"), hoverongaps=False,
            ))
            fig.update_layout(
                title=f"Deterministic {season} Temperature Forecast",
            ...
            if len(valid) > 0:
                col1.metric("Min Temp", f"{np.nanmin(valid):.1f} C")
                col2.metric("Mean Temp", f"{np.nanmean(valid):.1f} C")
                col3.metric("Max Temp", f"{np.nanmax(valid):.1f} C")
            ds.close()
        ...
        if img.exists():
            st.image(str(img), caption=f"{season} Temperature Forecast", width="stretch")
        else:
            st.info(f"No {season} temperature forecast data available. Run the seasonal TEMP pipeline.")
    ...
    if nc_prob.exists():
        try:
            ...
            fig = go.Figure(data=go.Heatmap(
                z=data.values, x=ds[lon_name].values, y=ds[lat_name].values,
                colorscale="RdBu_r", colorbar=dict(title="Probability"),
                zmin=0, zmax=1, hoverongaps=False,
            ))
            fig.update_layout(
                title=f"{season} Temperature Probability - {selected}",
                xaxis_title="Longitude", yaxis_title="Latitude", height=600,
                yaxis=dict(scaleanchor="x", scaleratio=1),
            )
            st.plotly_chart(fig, width="stretch")
        ...
        st.info(f"No probabilistic {season} temperature data available.")
