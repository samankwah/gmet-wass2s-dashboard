...
if forecast_dir and forecast_dir.exists():
    det_matches = sorted(forecast_dir.glob(f"*Det*TEMP*{season}*.nc"))
    if det_matches:
        nc_det = det_matches[0]
    prob_matches = sorted(forecast_dir.glob(f"*Prob*TEMP*{season}*.nc"))
    if prob_matches:
    ...
    if nc_det and nc_det.exists():
        ...
        if data is not None:
            fig = forecast_heatmap(data.values, lon, lat, P["colorscale"], P["colorbar_title"],
                                   f"Deterministic {season} Temperature Forecast")
        ...
        if forecast_dir and forecast_dir.exists():
            img_matches = sorted(forecast_dir.glob(f"*{season}*TEMP*.png"))
            if img_matches:
                st.image(str(img_matches[0]), caption=f"{season} Temperature Forecast", use_container_width=True)
            else:
                st.info(f"No {season} temperature forecast data available.")
        else:
            st.info(f"No {season} temperature forecast data available.")
else:
    if nc_prob and nc_prob.exists():
        ...
        if categories:
            ...
            fig = forecast_heatmap(categories[selected], lon, lat,
                                   "RdBu_r", "Probability", f"{season} Temperature Probability \u2014 {selected}",
                                   zmin=0, zmax=1)
            st.plotly_chart(fig, use_container_width=True)
    else:
