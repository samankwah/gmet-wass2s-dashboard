...
st.set_page_config(page_title="Late Dry Spell", page_icon=":desert:", layout="wide")

APP_DIR = Path(__file__).parent.parent
DATA_DIR = APP_DIR / "data" / "late_dry_spell"

st.title("2nd / Late Dry Spell Forecast")
st.markdown("Predicted timing of the second or late-season dry spell — critical for water management and crop planning.")
...
if forecast_type == "Deterministic":
    if nc_det.exists():
        try:
            ...
            fig.update_layout(
                title="Deterministic Late Dry Spell Forecast (Day of Year)",
        ...
        if img.exists():
            st.image(str(img), caption="Late Dry Spell Forecast", width="stretch")
        else:
            st.info("No late dry spell forecast data available yet. Run the pipeline to generate.")
else:
    if nc_prob.exists():
        try:
        ...
        st.info("No probabilistic late dry spell forecast data available yet.")
